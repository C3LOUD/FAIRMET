...
const ReferenceSearch = () => {
	...
	return (
		<Box as="section" maxW="71.25rem" id="reference-search" mb="4rem">
			...
			<HStack>
				<Text>{'Seek your air, build your wardrobe \uFF06 find your lifestyle'}</Text>
				<Spacer />
				<Text>{'Sort by\uFF1A'}</Text>
				<Select w="fit-content" h="fit-content" border="none" _focus={{ ring: 'none' }}>
			...
			{brands.length ? (
				<Flex w="100%" position="relative" gap="5rem" wrap="wrap">
					{[...new Array(rows)].map((_, index) => (
						<VStack flex="1" key={index}>
							{brands.map((brand, i: number) => {
								if (i % rows !== index || i > limit * rows - 1) return <></>;
								return <DictionaryCard key={brand.id} brand={brand} onClick={clickHandler} />;
							})}
						</VStack>
					))}

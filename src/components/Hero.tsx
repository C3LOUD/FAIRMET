...
const Hero = () => {
	...
	const leftHandler = (e: React.MouseEvent) => {
		...
		for (const [i, el] of [...ref.current.children].entries()) {
			if (el.getClientRects()[0].left >= containerRef.current?.getClientRects()[0].x!) {
				elToShow = i - 1;
				break;
			}
		}
		...
		const offset =
			ref.current.children[elToShow < 0 ? 0 : elToShow].getClientRects()[0].left -
	...
	const rightHandler = (e: React.MouseEvent) => {
		...
		for (const [i, el] of [...ref.current.children].entries()) {
			if (
				...
				containerRef.current?.offsetWidth! + containerRef.current?.getClientRects()[0].x!
			) {
				elToShow = i;
				break;
			}
					...
					>
						...
						>
							{heroCardList.map((heroCard) => (
								<GridItem key={heroCard?.id} position="relative" h="100%" minW="max-content">
									<Image src={heroCard?.image} alt={heroCard?.title} h="36rem" objectFit="cover" />
									<Text
										bg="white"
										position="absolute"
										bottom="5"
